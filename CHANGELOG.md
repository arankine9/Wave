...
### Removed
- **LLM / Ollama cleanup stage removed entirely.** Wave now relies solely on the pure-Swift `DeterministicCleanup` pass (disfluency removal, spoken-symbol substitution for code, identifier reassembly, spacing/casing) that always ran ahead of the model. Gone: `OllamaClient`, `OllamaHealth`/`OllamaHealthProbe`, `CleanupClient`, `CleanupPipeline`, `CleanupResult`, `CleanupError`, `IdentityCache`, `SystemPrompt`, the `CleanupMode` preference, the `cleanupModel`/`ollamaURL` preferences, the `WAVE_CLEANUP_MODEL`/`WAVE_OLLAMA_URL`/`WAVE_CLEANUP_MODE` env vars, `SkipGate.shouldSkipCleanup`, and `AppPaths.identityCacheURL`. The Settings → Cleanup section (mode picker, Ollama model/URL fields, health probe) is removed, the History panel no longer shows the original/cleaned before-after toggle or the Cleaned/Raw badge, and the onboarding "cleaned text" page is reframed around plain paste. `scripts/judge.sh`, the cleanup-pairs/identifier-spelling/hallucination-audit fixtures, and the LLM-only tests (`CleanupPipelineTests`, `CleanupModeTests`, `OllamaHealthProbeTests`, `SystemPromptTests`, `TokenBudgetBenchmarkTests`, `HallucinationFixtureTests`, `IdentityCacheTests`, `SkipGateTests`, `FixtureGateTests`) are deleted. `DictationTrace`/`HistoryLine` drop the `path`/`inputTokens`/`outputTokens` fields (existing `history.jsonl` still decodes; the extra keys are ignored).

### Changed
- **STT swapped to FluidAudio's Parakeet TDT v2 (CoreML/ANE).** Single backend, English-only, highest recall on the M3 Pro. New `ParakeetBackend` records via AVAudioEngine with Apple's system voice processing enabled on the input node (AEC + noise/voice suppression), then transcribes a 16 kHz mono Float buffer through `AsrManager.transcribe(_:decoderState:)`. Model is downloaded lazily on the first `startSession()` and pre-warmed in the background at app launch. The backend picker, Whisper.cpp binary, GGML model, Voxtral Python sidecar, `STTBackendKind`/`STTBackendFactory`, and the `WAVE_STT_BACKEND`/`WAVE_WHISPER_*`/`WAVE_VOXTRAL_PYTHON` env vars are all gone — `AppDelegate` instantiates `ParakeetBackend()` directly.
- **Bulletproof Fn-key capture.** Wave now silently reclaims the Fn (Globe) key on every launch — no logout, no System Settings detour, no onboarding step. The recipe: write `AppleFnUsageType=0` to `com.apple.HIToolbox` AND post the `com.apple.KeyboardUIModeDidChange` distributed notification (the signal System Settings itself fires via the HIServices XPC service when you flip the dropdown). Plist write alone is NOT enough — HIToolbox caches the value in-memory and only refreshes when this specific notification fires. Discovered by tracing `log stream` during a manual dropdown flip and bisecting against the notification names defined in `com.apple.hiservices-xpcservice`. The `FnKeyMonitor` event tap was also moved from `kCGSessionEventTap` to `kCGHIDEventTap` (strictly lower in the event-flow chain) and gained a 2-second watchdog that re-enables the tap if macOS quiesces it for either timeout or user-input reasons. Onboarding's Fn-key permission step, the dropdown overlay variant, the `.fnKey` permission prompt, and `PermissionDropdownRow` are all removed.

### Fixed
- **Clipboard race could paste your previous clipboard instead of the dictation.** In paste mode, `ClipboardPaster` snapshots the clipboard, writes the dictated text, fires Cmd+V, then restores the snapshot. Cmd+V is consumed *asynchronously* by the frontmost app, so on a slow/busy app the 0.15s restore fired before the app read the pasteboard — and it pasted the stale snapshot (e.g. a file name copied in Finder) instead of the transcript. Timing-dependent, so it surfaced only on slower machines. Restore window widened 0.15s → 0.6s (`ClipboardPaster.defaultRestoreDelay`); the existing `changeCount` guard already ensures we only restore when our text is still exactly what's on the clipboard. The visible paste is unaffected — restore stays deferred and off the hot path. Users who want a structural guarantee can still pick "Type each character" in Settings.

## 2026-05-10

### Added
- **Bootstrap.** Native Swift `Wave` exec + `WaveCore` library, NSStatusItem with `waveform` SF Symbol, `LSUIElement = true` (no dock icon), `scripts/build-app.sh` producing an ad-hoc-signed `.app`.
...
- **Hotkey state machine.** Hold-to-dictate (≥ 250 ms) and double-tap-to-lock semantics on an injectable `WaveClock`. Six deterministic tests.
...
- **History.** `HistoryLogger` JSON-lines writer in `~/Library/Application Support/Wave/history.jsonl`; SwiftUI `HistoryWindowController` showing the last 50 entries with timing + SKIPPED/CLEANED badges.
...
- **Release pipeline.** `scripts/release.sh` builds, codesigns with `WAVE_SIGNING_IDENTITY`, notarizes via `notarytool`, staples, produces `dist/Wave.dmg`. Falls back to unsigned DMG cleanly when env is missing.
...
- **Apple Developer signing + notarization.** Pipeline is in place; needs the operator to set `WAVE_SIGNING_IDENTITY`, `APPLE_ID`, `APPLE_APP_SPECIFIC_PASSWORD`, `APPLE_TEAM_ID` then `bash scripts/release.sh`.
