...
- New `Sources/BeckCore/STT/ParakeetBackend.swift`, conforming to the existing `STTBackend` protocol.
...
### Code
- `Sources/BeckCore/STT/WhisperCppBackend.swift`
- `Sources/BeckCore/STT/VoxtralBackend.swift`
- `Sources/BeckCore/STT/STTBackendFactory.swift` (or simplify to nothing — direct `ParakeetBackend()` instantiation in `AppDelegate`).
...
- `STTBackendKind` enum + related fields/env vars in `Preferences.swift`:
  - `BECK_STT_BACKEND`, `BECK_WHISPER_BIN`, `BECK_WHISPER_MODEL`, `BECK_VOXTRAL_PYTHON`.
...
### Tests
- `Tests/BeckCoreTests/WhisperCppBackendTests.swift`
- `Tests/BeckCoreTests/STTBackendFactoryTests.swift` (delete; replace with a minimal Parakeet smoke test if useful — skipped unless models present).
