...
enum Diagnostics {
    ...
    static func render(prefs: PreferencesStore) async -> String {
        ...
        let lines = [
            "Voxflow diagnostics",
            "===================",
            "version:        \(VoxflowVersion.current)",
            ...
            "  input monitoring:  \(perms.inputMonitoring.rawValue)",
            "  accessibility:     \(perms.accessibility.rawValue)",
            "",
            "Ollama:           \(ollamaLine)",
            "Voxtral env set:  \(ProcessInfo.processInfo.environment["VOXFLOW_VOXTRAL_PYTHON"] ?? "no")",
            "",
