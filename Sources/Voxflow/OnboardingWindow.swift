    ...
    var body: some View {
        VStack(spacing: 18) {
            VStack(spacing: 6) {
                ...
                Text("Welcome to Voxflow").font(.title2.weight(.semibold))
            ...
            VStack(spacing: 10) {
                ...
                row("Input Monitoring", "Detects the global Fn-key dictation hotkey",
                    grant: snapshot.inputMonitoring, pane: .inputMonitoring)
                row("Accessibility", "Pastes cleaned text into the focused app",
            ...
            VStack(spacing: 6) {
                Text("Tip: double-tap Fn to lock dictation on, single tap to stop.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
                Text("Voxflow lives in the menu bar near the battery — there's no dock icon.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
            .multilineTextAlignment(.center)
