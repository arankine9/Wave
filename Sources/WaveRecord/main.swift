...
let runner = Task {
    if micStatus != .authorized {
        FileHandle.standardError.write(Data("[wave-record] requesting mic permission...\n".utf8))
        let granted = await AVCaptureDevice.requestAccess(for: .audio)
        FileHandle.standardError.write(Data("[wave-record] mic permission granted: \(granted)\n".utf8))
        if !granted {
            FileHandle.standardError.write(Data("[wave-record] ABORT: mic not authorized. Grant via System Settings → Privacy & Security → Microphone for Terminal (or whatever runs this binary).\n".utf8))
            exit(1)
        }
    }
    do {
        let session = try await backend.startSession()
        FileHandle.standardError.write(Data("[wave-record] session started — speak now\n".utf8))
        try await Task.sleep(nanoseconds: UInt64(seconds * 1_000_000_000))
        FileHandle.standardError.write(Data("[wave-record] recording done, transcribing\n".utf8))
        let text = try await session.finalize()
        print(text)
        FileHandle.standardError.write(Data("[wave-record] done\n".utf8))
        exit(0)
    } catch {
        FileHandle.standardError.write(Data("[wave-record] FAILED: \(error)\n".utf8))
        exit(1)
    }
}
