...
/// FluidAudio cache is empty, so it's gated behind the `BECK_RUN_PARAKEET_TEST`
...
final class ParakeetBackendTests: XCTestCase {
    func testParakeetTranscribesSynthesizedSpeech() async throws {
        ...
        guard env["BECK_RUN_PARAKEET_TEST"] == "1" else {
            throw XCTSkip("set BECK_RUN_PARAKEET_TEST=1 to run the Parakeet integration test (downloads ~hundreds of MB on first run)")
        }
    ...
    private func synthesize16kFloat(text: String) throws -> [Float] {
        ...
        let aiff = tmp.appendingPathComponent("beck-parakeet-\(UUID().uuidString).aiff")
        let wav = tmp.appendingPathComponent("beck-parakeet-\(UUID().uuidString).wav")
        defer {
