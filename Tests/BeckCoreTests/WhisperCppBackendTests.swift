...
final class WhisperCppBackendTests: XCTestCase {
    func testTranscribesSynthesizedSpeech() async throws {
        guard WhisperCppBackend.isAvailable() else {
            throw XCTSkip("whisper.cpp not installed; install with `brew install whisper-cpp` and download a model to ~/.beck/models/ggml-base.en.bin")
        }

        let phrase = "Hello world this is a Beck test"
        ...
        XCTAssertTrue(
            normalized.contains("hello") && normalized.contains("beck"),
            "expected transcript to mention 'hello' and 'beck'; got: \(transcript)"
        )
    }
    ...
    private func synthesizeWav(text: String) throws -> URL {
        ...
        let aiff = tmp.appendingPathComponent("beck-test-\(UUID().uuidString).aiff")
        let wav = tmp.appendingPathComponent("beck-test-\(UUID().uuidString).wav")
