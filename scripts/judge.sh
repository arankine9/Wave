...
#
# Requirements: Ollama daemon reachable at $VOXFLOW_OLLAMA_URL with the
# cleanup model and the judge model both pulled. `jq` for JSON munging.
#
# Usage: scripts/judge.sh [--cleanup MODEL] [--judge MODEL]
set -euo pipefail

CLEANUP_MODEL="${VOXFLOW_CLEANUP_MODEL:-qwen2.5-coder:7b-instruct}"
JUDGE_MODEL="${VOXFLOW_JUDGE_MODEL:-qwen2.5:7b-instruct}"
OLLAMA_URL="${VOXFLOW_OLLAMA_URL:-http://127.0.0.1:11434}"
